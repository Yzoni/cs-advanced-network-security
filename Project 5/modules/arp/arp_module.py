...
class ARPModule(IPSModule):
    ...
    def receive_packet(self, pkt, ts):
        ...
        if pkt[ARP].op == ARP.who_has:  # Request
            if not self.request_mac_hwdst_is_not_zero(pkt):
                return NoticeResponse('Request ARP MAC address is not 00:00...', {'pkt': repr(pkt)})
            if self.db.request_sender_should_have_ip(pkt[ARP].hwsrc, pkt[ARP].pdst):
                return NoticeResponse('Requester should have known the IP already', {'pkt': repr(pkt)})
            if not self.is_send_to_broadcast(pkt):
                return NoticeResponse('ARP Request is not to broadcast', {'pkt': repr(pkt)})
            if not self.request_linklayer_address_matches_arp(pkt):
                return NoticeResponse('Link layer MAC does not match ARP request MAC', {'pkt': repr(pkt)})
            ...
            if self.is_send_to_broadcast(pkt):
                return NoticeResponse('ARP reply is not to unicast', {'pkt': repr(pkt)})
            if not self.reply_linklayer_address_matches_arp(pkt):
                return NoticeResponse('Link layer MAC does not match ARP reply MAC', {'pkt': repr(pkt)})
    ...
    def is_send_to_broadcast(self, pkt):
        return pkt.dst == 'ff:ff:ff:ff:ff:ff'

    def has_valid_arp_src_mac_address(self, pkt):
        return is_valid_mac_address(pkt[ARP].hwsrc)

    def has_valid_arp_dst_mac_address(self, pkt):
